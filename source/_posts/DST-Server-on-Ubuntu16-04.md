...
* 本地打开DST，创建一个在线世界，Mod什么都设置好，点击开始进游戏，然后关闭游戏。

找到存档 Linux存档位置：`~/.klei/DoNotStarveTogether` Windows存档位置：`%HOMEPATH%\Klei\DoNotStarveTogether\`。

将对应的Cluster_X覆盖到云主机的下面目录：`~/.klei/DoNotStarveserver_1/`。

将上一步获得的`cluster_token.txt`放到Cluster_X目录下。
...
几分钟后即可在DST的online列表中搜到你的服务器。

> 我上面写的教程没开Cave，你可以通过看视频把Cave服务打开，过程类似。